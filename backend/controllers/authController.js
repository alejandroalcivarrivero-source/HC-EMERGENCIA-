...
exports.login = async (req, res) => {
  ...
  try {
    const usuario = await Usuario.findOne({ where: { cedula: String(cedula) } });
    if (!usuario) {
    ...
    if (usuario.estado_cuenta === 'BLOQUEADO') {
      // Auto-desbloqueo después de 20 minutos
      const tiempoBloqueo = new Date(usuario.ultimo_bloqueo);
      ...
      if (diferenciaMinutos >= 20) {
        ...
        const minutosRestantes = Math.ceil(20 - diferenciaMinutos);
        return res.status(403).json({
          mensaje: `Tu cuenta está bloqueada temporalmente. Intenta nuevamente en ${minutosRestantes} minutos o contacta al administrador.`
        });
      }
    }
    ...
    if (!valida) {
      ...
      if (nuevosIntentos >= 10) {
        return res.status(403).json({
          mensaje: 'Has alcanzado el límite de intentos (10). Tu cuenta ha sido bloqueada por 20 minutos.'
        });
      }
      ...
      return res.status(401).json({
        ...
        max_intentos: 10,
        intentosRestantes: 10 - nuevosIntentos
      });
    }
...
exports.recuperar = async (req, res) => {
  ...
  try {
    ...
    let correoDestino;
    if (destino === 'institucional') {
        correoDestino = usuario.correo;
    } else if (destino === 'alternativo') {
        correoDestino = usuario.correo_alternativo;
    }

    if (!correoDestino) {
        return res.status(400).json({ mensaje: `El usuario no tiene un correo de tipo '${destino}' registrado.` });
    }
    ...
    console.log('===========================================');
    console.log(`EMERGENCIA OTP [${cedula}] para ${correoDestino}: ${otp}`);
    ...
    try {
      await enviarCorreo(
        correoDestino,
        'Recuperación de contraseña y Desbloqueo de Cuenta',
        `
      ...
      console.error('Error al enviar correo (Modo Consola Activo):', mailError.message);
      await LogCorreo.create({
        correo_destino: correoDestino,
        tipo: 'OTP_RECUPERACION',
        estado: 'FALLIDO',
      ...
      return res.status(500).json({
        mensaje: 'Error de conexión con servidor de correo. El código ha sido generado en modo de emergencia (Consulte con soporte).',
        token_otp: token,
    ...
    res.status(200).json({
      mensaje: `Se ha enviado un código y un enlace a tu correo ${destino}.`,
      token_otp: token
    });
  } catch (error) {
    console.error('Error en la función de recuperar:', error);
...
// SOLICITAR OTP (Método B - Zimbra/Alternativo)
exports.solicitarOtpRecuperacion = async (req, res) => {
    const { cedula, destino } = req.body;
    try {
        if (!destino) {
            return res.status(400).json({ mensaje: 'Debe especificar el destino del correo (institucional o alternativo).' });
        }
        const usuario = await Usuario.findOne({ where: { cedula } });
        if (!usuario) {
            return res.status(404).json({ mensaje: 'Usuario no encontrado con esa cédula.' });
        }
        let correoDestino;
        if (destino === 'institucional') {
            correoDestino = usuario.correo;
        ...
        try {
            await enviarCorreo(
                ...
                `
                    <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
                        ...
                        <p>Se ha solicitado un código de recuperación a su correo ${destino}.</p>
            ...
            console.error('Error crítico en Nodemailer:', mailError.message);
             await LogCorreo.create({
                correo_destino: correoDestino,
                tipo: 'OTP_RECUPERACION',
                estado: 'FALLIDO',
            ...
            return res.status(500).json({
                mensaje: 'Error de conexión con el servidor de correo. Se generó un código de respaldo en la consola del servidor (Contacte a Soporte).',
                token_otp: token,
                bypass: true
            });
        }

        res.json({ mensaje: 'Código de recuperación enviado exitosamente.', token_otp: token });
    } catch (error) {
        console.error('Error crítico al solicitar OTP:', error);
...
// 1. SOLICITAR OTP PARA CAMBIO DE CONTRASEÑA (Paso 3)
exports.solicitarOtpCambioContrasena = async (req, res) => {
    const { cedula, tipoCorreo } = req.body; // 'correo' o 'correo_alternativo'

    if (!tipoCorreo || !cedula) {
        return res.status(400).json({ message: 'Debe especificar la cédula y el tipo de correo para enviar el código.' });
    }
    ...
    try {
        ...
        const correoDestino = usuario[tipoCorreo];
        if (!correoDestino) {
            const nombreCampo = tipoCorreo === 'correo' ? 'principal' : 'alternativo';
            return res.status(404).json({ message: `No se encontró un correo ${nombreCampo} asociado a la cuenta.` });
        }

        const otp = otpService.generarOTP();
        // Guardar OTP en un token temporal de 5 min para mantener el estándar de seguridad institucional
        const token_otp = jwt.sign({ id: usuario.id, otp }, JWT_SECRET, { expiresIn: '5m' });

        try {
            await emailService.sendOTP(correoDestino, otp);
            console.log(`✅ OTP enviado a [${correoDestino}]`);
        } catch (mailError) {
            // Mantenemos el log de error simplificado
            console.error(`❌ Error al enviar OTP a [${correoDestino}]:`, mailError.message);
            
            // En caso de error, devolvemos el token para permitir flujo de soporte si es necesario,
            // pero indicando que falló el envío.
            return res.json({
                mensaje: 'Error de red con el servidor de correo. Su código ha sido generado internamente por soporte.',
                token_otp,
                modo_emergencia: true
            });
        }

        res.json({ mensaje: `Código de verificación enviado a ${correoDestino}.`, token_otp });
    } catch (error) {
        console.error('Error al solicitar OTP cambio clave:', error);
        res.status(500).json({ message: 'Error al enviar el código de verificación.' });
    }
};

// 2. CAMBIAR CONTRASEÑA CON VERIFICACIÓN OTP (Paso 3)
exports.cambiarContrasena = async (req, res) => {
  const { currentPassword, newPassword, otp, token_otp } = req.body;
  const userId = req.usuario ? req.usuario.id : req.userId;
  ...
  try {
    ...
    try {
        const decoded = jwt.verify(token_otp, JWT_SECRET);
        if (decoded.id !== usuario.id || !otpService.verificarOTP(otp, decoded.otp)) {
            return res.status(400).json({ message: 'El código de verificación es incorrecto o ha expirado.' });
        }
    } catch (err) {
        return res.status(400).json({ message: 'La sesión de verificación ha expirado.' });
    }

    // B. Validar la contraseña actual
    const isMatch = await bcrypt.compare(currentPassword, usuario.contrasena);
    if (!isMatch) {
      return res.status(400).json({ message: 'La contraseña actual es incorrecta.' });
    }

    // C. Validar que la nueva contraseña no sea igual a la anterior
    const isSameAsOld = await bcrypt.compare(newPassword, usuario.contrasena);
    if (isSameAsOld) {
      return res.status(400).json({ message: 'La nueva contraseña no puede ser igual a la anterior.' });
    }

    // D. Hashear y guardar la nueva contraseña
    const salt = await bcrypt.genSalt(10);
