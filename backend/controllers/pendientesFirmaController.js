  ...
  try {
    ...
    if (atencion) {
      ...
      const diag = await DetalleDiagnostico.findOne({
        where: { atencionEmergenciaId: atencion.id, tipo_diagnostico: 'DEFINITIVO' }, // Priorizar definitivo
        include: [{ model: CatCie10, as: 'CIE10' }],
        order: [['createdAt', 'DESC']]
      }) || await DetalleDiagnostico.findOne({
        where: { atencionEmergenciaId: atencion.id, tipo_diagnostico: 'PRESUNTIVO' },
        include: [{ model: CatCie10, as: 'CIE10' }],
        order: [['createdAt', 'DESC']]
      });
