...
const OrdenExamenForm = ({ admisionId, onClose, onSaveSuccess }) => {
  ...
  useEffect(() => {
    const fetchOrdenes = async () => {
      try {
        ...
        if (!token) return;
        const response = await axios.get(`http://localhost:3001/api/ordenes-examen/${admisionId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setOrdenesExistentes(response.data);
      } catch (err) {
        console.error('Error al cargar órdenes de examen existentes:', err);
        setError('Error al cargar órdenes de examen existentes.');
      }
    };
    fetchOrdenes();
  }, [admisionId]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('http://localhost:3001/api/ordenes-examen', dataToSend, {
        headers: { Authorization: `Bearer ${token}` }
      });

      alert('Orden de examen guardada exitosamente.');
      setTipoExamen('');
      setObservaciones('');
      setFirmaElectronica('');
      ...
      console.error('Error al guardar la orden de examen:', err);
      if (err.response && err.response.status === 401) {
        setError('Firma electrónica inválida. Por favor, verifique su contraseña.');
      } else {
        setError('Error al guardar la orden de examen. Intente de nuevo.');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="p-4">
      <h2 className="text-xl font-bold mb-4">Orden de Examen (Formulario 010)</h2>
        ...
        <div className="mb-6">
          ...
          {ordenesExistentes.map((orden) => (
            <div key={orden.id} className="border p-3 mb-2 rounded-md bg-gray-50">
              ...
              <p><strong>Tipo de Examen:</strong> {orden.tipoExamen}</p>
      ...
      <form onSubmit={handleSubmit}>
        <div className="mb-4">
          <label htmlFor="tipoExamen" className="block text-gray-700 text-sm font-bold mb-2">Tipo de Examen:</label>
          <input
            type="text"
            id="tipoExamen"
            name="tipoExamen"
        ...
        <div className="flex items-center justify-between">
          <button
            ...
            disabled={loading}
          >
            {loading ? 'Guardando...' : 'Guardar Orden de Examen'}
          </button>
          <button
            type="button"
            onClick={onClose}
...
export default OrdenExamenForm;